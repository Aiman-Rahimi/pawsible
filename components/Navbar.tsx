    ...
    <nav style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }} className="hidden md:flex">
          ...
          {session && (
            <Link href="/favorites" style={linkStyle("/favorites")}
              onMouseEnter={e => { if (!pathname.startsWith("/favorites")) { e.currentTarget.style.background = "rgba(0,0,0,0.06)"; e.currentTarget.style.color = "var(--ink)"; }}}
              onMouseLeave={e => { if (!pathname.startsWith("/favorites")) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "var(--muted)"; }}}>
              ❤️ Saved
            </Link>
          )}
          {isAdmin && (
                ...
                {dropOpen && (
                  <div style={{
                    ...
                    {[
                      ...
                      { href: "/favorites",     label: "Saved Pets",  Icon: Heart      },
                    ].map(item => (
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div style={{ background: "var(--paper)", borderTop: "1px solid var(--border)", padding: "12px 2rem 20px" }} className="md:hidden">
          {navLinks.map(l => (
            <Link key={l.href} href={l.href} onClick={() => setMobileOpen(false)} style={{
              ...
              textDecoration: "none", borderBottom: "1px solid var(--border)", fontFamily: "var(--font-body)",
            }}>
              {l.label}
            </Link>
          ))}
          <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
            {session ? (
              <button onClick={() => signOut({ callbackUrl: "/" })} className="btn-outline" style={{ flex: 1 }}>Sign Out</button>
            ) : (
              <>
